...
const ES_LESSONS = [
  { url: "/es/strategies/multi-timeframe/lecon1" },
  { url: "/es/strategies/macro-trading/lecon1" },
];

for (const { url } of ES_LESSONS) {
  test(`ES strategies — ${url}`, async ({ page }) => {
...
// FR non-régression
const FR_LESSONS = [
  { url: "/fr/strategies/multi-timeframe/lecon1" },
  { url: "/fr/strategies/macro-trading/lecon1" },
];

for (const { url } of FR_LESSONS) {
  test(`FR strategies — ${url} reste en FR`, async ({ page }) => {
