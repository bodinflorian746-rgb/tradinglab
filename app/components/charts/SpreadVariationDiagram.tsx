...
// Mobile curve — simplified, only 7 sample points to convey shape
const MOBILE_CURVE: [number, number][] = [
  [10, 30],   // 0h  haut
  [40, 12],   // 2h  MAX
  [80, 35],   // 5h
  [140, 65],  // 9h
  [200, 70],  // 13h MIN
  [260, 50],  // 18h
  [310, 16],  // 23h  remonte haut
];
const mobileCurve = MOBILE_CURVE.map(([x, y]) => `${x},${y}`).join(' ');
    ...
    <div className={`bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden ${className}`}>
      ...
      <div className="sm:hidden p-4 space-y-3">
        <svg viewBox="0 0 320 90" width="100%" fill="none" aria-label="Spread EUR/USD sur 24h">
          ...
          {/* Courbe simplifiée */}
          <polyline points={mobileCurve} stroke="#a1a1aa" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
          {/* MAX point */}
          <circle cx={40} cy={12} r="6" fill="#09090b" stroke="#f87171" strokeWidth="2.5" />
          {/* MIN point */}
          <circle cx={200} cy={70} r="6" fill="#09090b" stroke="#34d399" strokeWidth="2.5" />
          {/* Axe X minimal */}
          <line x1={0} y1={84} x2={320} y2={84} stroke="#3f3f46" strokeWidth="1" />
          <text x={5}   y={94} fontSize="9" fill="#71717a">0h</text>
          <text x={155} y={94} fontSize="9" fill="#71717a" textAnchor="middle">12h</text>
          <text x={315} y={94} fontSize="9" fill="#71717a" textAnchor="end">24h</text>
        </svg>

        {/* Bullets explicatifs */}
        <ul className="space-y-2 text-[13px] leading-snug">
          <li className="flex items-start gap-2.5">
            <span className="shrink-0 w-5 h-5 rounded-full bg-red-500/10 border-2 border-red-400 flex items-center justify-center mt-0.5">
              <span className="w-1.5 h-1.5 rounded-full bg-red-400" />
            </span>
            <span className="text-white"><span className="font-semibold">8 pts maximum vers 2h</span> — heures creuses, faible liquidité</span>
          </li>
          <li className="flex items-start gap-2.5">
            <span className="shrink-0 w-5 h-5 rounded-full bg-emerald-500/10 border-2 border-emerald-400 flex items-center justify-center mt-0.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
            </span>
            <span className="text-white"><span className="font-semibold">1 pt minimum vers 13h</span> — pic de liquidité (London + NY)</span>
          </li>
          <li className="flex items-start gap-2.5 pt-1 border-t border-zinc-800/50">
            <span className="shrink-0 text-zinc-300 font-bold mt-0.5">→</span>
            <span className="text-zinc-300">Spread serré = bonne fenêtre. Spread large = éviter les petits objectifs.</span>
          </li>
        </ul>
      </div>
    </div>
