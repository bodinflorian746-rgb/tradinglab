export const DollarHubDiagram = ({ locale = "fr" }: { locale?: "fr" | "es" } = {}) => {
  const t = locale === "es"
    ? {
        ...
        subtitle: "5 mercados mayores, 1 sola moneda pivote",
        forex: "FOREX",
        or: "ORO",
        crypto: "CRYPTO",
        indices: "INDICES",
        matieresPrem: "MATERIAS PRIM.",
        matieresPremFull: "MATERIAS PRIMAS",
        petroleCuivre: "Petróleo, cobre",
        forceDollar: "Fuerza del dólar",
        dxyUp: "DXY ↑ = presión",
        dxyDown: "DXY ↓ = respiro",
    ...
    : {
        ...
        subtitle: "5 marchés majeurs, 1 seule devise pivot",
        forex: "FOREX",
        or: "OR",
        crypto: "CRYPTO",
        indices: "INDICES",
        matieresPrem: "MATIÈRES PREM.",
        matieresPremFull: "MATIÈRES PREMIÈRES",
        petroleCuivre: "Pétrole, cuivre",
        forceDollar: "Force du dollar",
        dxyUp: "DXY ↑ = pression",
        dxyDown: "DXY ↓ = respiration",
        footer: "Le dollar est au centre. Quand il bouge, tout réagit.",
        mobileEffectUp: "= pression",
        mobileEffectDown: "= respiration",
      };
  return (
    <div>
    <svg
    ...
    >
      ...
      <text x="400" y="125" fill="#d4d4d8" fontSize="12" fontWeight="700" textAnchor="middle">{t.forex}</text>
      ...
      <text x="595" y="165" fill="#d4d4d8" fontSize="12" fontWeight="700" textAnchor="middle">{t.or}</text>
      ...
      <text x="595" y="300" fill="#d4d4d8" fontSize="12" fontWeight="700" textAnchor="middle">{t.crypto}</text>
      ...
      <text x="205" y="300" fill="#d4d4d8" fontSize="12" fontWeight="700" textAnchor="middle">{t.indices}</text>
      ...
      <text x="205" y="165" fill="#d4d4d8" fontSize="12" fontWeight="700" textAnchor="middle">{t.matieresPrem}</text>
      <text x="205" y="185" fill="#a1a1aa" fontSize="10" textAnchor="middle">{t.petroleCuivre}</text>

      {/* Layer 5 — Hub central (DXY/USD) */}
      <circle cx="400" cy="235" r="70" fill="#27272a" stroke="#60a5fa" strokeWidth="3" />
      <text x="400" y="212" fill="#a1a1aa" fontSize="10" fontStyle="italic" textAnchor="middle">{t.forceDollar}</text>
      ...
      <text x="335" y="395" fill="#f87171" fontSize="11" fontWeight="700" textAnchor="middle">{t.dxyUp}</text>

      {/* Droite — DXY ↓ */}
      <rect x="420" y="380" width="110" height="22" rx="3" fill="#09090b" fillOpacity="0.85" />
      <text x="475" y="395" fill="#34d399" fontSize="11" fontWeight="700" textAnchor="middle">{t.dxyDown}</text>
    ...
    <div className="sm:hidden bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden p-4 space-y-3">
      ...
      <div className="rounded-2xl border-2 border-blue-400 bg-zinc-800 p-4 text-center">
        <p className="text-[11px] text-zinc-400 italic">{t.forceDollar}</p>
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          { name: t.forex, desc: "EUR/USD, GBP/USD" },
          { name: t.or, desc: "XAU/USD" },
          { name: t.crypto, desc: "BTC/USD" },
          { name: t.indices, desc: "NASDAQ, S&P500" },
        ].map((sat) => (
        ...
        <div className="col-span-2 rounded-lg border border-zinc-700 bg-zinc-800/60 p-2.5 text-center">
          <p className="text-[13px] font-bold text-zinc-200">{t.matieresPremFull}</p>
          <p className="text-[11px] text-zinc-400 mt-0.5">{t.petroleCuivre}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-zinc-800">
        <div className="rounded-lg bg-red-500/10 border border-red-500/30 p-2 text-center">
          ...
          <p className="text-[12px] text-zinc-300 mt-0.5">{t.mobileEffectUp}</p>
        </div>
        <div className="rounded-lg bg-emerald-500/10 border border-emerald-500/30 p-2 text-center">
          ...
          <p className="text-[12px] text-zinc-300 mt-0.5">{t.mobileEffectDown}</p>
        </div>
      </div>
