// Page post-inscription grand public.
// Affichée après un signUp : on a envoyé un code d'activation par email.
// Invite l'utilisateur à aller chercher le code et à l'entrer sur /activer-code.
// Bilingue FR/ES via le dictionnaire common (clé verifyEmail).
...
export default async function VerifyEmailPage({
  ...
  const t = (await getDictionary(locale, "common")).verifyEmail;
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center">
        ...
        <p className="text-sm text-zinc-400 leading-relaxed mb-1">{t.sentTo}</p>
        {email && (
          <p className="text-sm font-semibold text-white mb-4 break-all">{email}</p>
        )}

        <p className="text-sm text-zinc-400 leading-relaxed mb-2">{t.body}</p>
