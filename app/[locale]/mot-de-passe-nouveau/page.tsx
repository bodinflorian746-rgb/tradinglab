// Page de saisie d'un nouveau mot de passe après recovery.
// Atterrissage typique : user vient de cliquer le lien reçu par mail →
// /[locale]/auth/callback a échangé le code → session recovery active.
//
// Si pas de session (lien périmé / accès direct), on affiche un message
// d'erreur invitant à redemander le lien.
...
export default async function NewPasswordPage({
  ...
  const t = (await getDictionary(locale, "common")).newPassword;

  // Vérifie qu'on a bien une session (recovery ou autre).
      ...
      <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl p-8">
        ...
        {user ? (
          ...
          <p className="text-sm text-zinc-500 mt-6 text-center">
            ...
            >
              {(await getDictionary(locale, "common")).resetPassword.title}
            </Link>
          </p>
        )}
      </div>
