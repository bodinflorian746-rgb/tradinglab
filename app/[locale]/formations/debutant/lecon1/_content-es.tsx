...
import { LESSONS_ES as LESSONS } from "@/lib/lessons-es";
...
const PHASE_LABELS  = ["Lectura", "Para recordar", "Ejercicio", "Quiz"];
          ...
          <div className="flex items-center gap-3 mb-4 pb-4 border-b border-zinc-800/60">
            ...
            <div>
              <p className="text-xs font-bold text-white leading-tight">
                {pct === 100 ? "¡Terminado!" : "En curso"}
              </p>
              <p className="text-[10px] text-zinc-600 mt-0.5">
                {phases.filter(Boolean).length} / {phases.length} etapas
              </p>
            </div>
          </div>
    ...
    <>
      ...
      <div className="hidden sm:grid mt-5 grid-cols-2 gap-3">
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3">
          <p className="text-[10px] font-bold text-emerald-400 text-center mb-2">Tienes razón ↑</p>
          <svg viewBox="0 0 140 140" className="w-full" fill="none">
            ...
            <text x="69" y="138" fontSize="10" fill="#10b981" textAnchor="middle" fontWeight="700">+3 000 $ de subida ✔</text>
          </svg>
        </div>
        <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-3">
          <p className="text-[10px] font-bold text-red-400 text-center mb-2">Te equivocas ↓</p>
          <svg viewBox="0 0 140 140" className="w-full" fill="none">
            <line x1="12" y1="105" x2="128" y2="105" stroke="#3f3f46" strokeWidth="1" />
            ...
            <text x="16" y="24" fontSize="8" fill="#a1a1aa">78 000 $ (compra)</text>
            <text x="69" y="102" fontSize="8" fill="#ef4444" textAnchor="middle">76 500 $</text>
            <text x="69" y="138" fontSize="10" fill="#ef4444" textAnchor="middle" fontWeight="700">−1 500 $ de bajada ✖</text>
          </svg>
        </div>
      </div>
        ...
        <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/5 p-4">
          ...
          <div className="grid grid-cols-2 gap-2 mb-2">
            ...
            <div className="rounded-lg bg-emerald-500/10 border border-emerald-500/30 px-3 py-2">
              <p className="text-[10px] text-emerald-400 uppercase tracking-wide">Venta</p>
              <p className="text-[15px] font-bold text-emerald-400 font-mono">81 000 $</p>
            </div>
          </div>
          <p className="text-center text-[14px] font-bold text-emerald-400">+3 000 $ de subida ✔</p>
        </div>
        ...
        <div className="rounded-xl border border-red-500/25 bg-red-500/5 p-4">
          ...
          <div className="grid grid-cols-2 gap-2 mb-2">
            ...
            <div className="rounded-lg bg-red-500/10 border border-red-500/30 px-3 py-2">
              <p className="text-[10px] text-red-400 uppercase tracking-wide">Venta</p>
              <p className="text-[15px] font-bold text-red-400 font-mono">76 500 $</p>
            </div>
          </div>
          <p className="text-center text-[14px] font-bold text-red-400">−1 500 $ de bajada ✖</p>
        </div>
      </div>
    </>
  );
...
function KeyPointsCard({ points, onComplete }: { points: string[]; onComplete: () => void }) {
  ...
  return (
    <div>
      <p className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest mb-2 px-1">Para recordar</p>
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold text-white">Puntos clave</span>
          </div>
        ...
        {!allChecked && (
          <p className="mt-4 text-center text-xs text-zinc-700">
            Marca cada punto para confirmar que lo has asimilado
          </p>
        )}
      </div>
    </div>
...
function ExerciceCard({ exercise, onComplete }: { exercise: { title: string; steps: string[] }; onComplete: () => void }) {
  ...
  return (
    <div>
      <p className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest mb-2 px-1">Ejercicio práctico</p>
      <div className={`rounded-2xl border p-6 transition-all duration-500 ${done ? "bg-zinc-900/30 border-zinc-800/50" : "bg-zinc-900/50 border-zinc-800"}`}>
        <div className="flex items-center justify-between mb-4">
          ...
          {done && <span className="text-xs font-medium text-emerald-400">Completado ✓</span>}
        </div>
        ...
        {!done ? (
          <>
            ...
            >
              {allChecked ? "Marcar como completado" : `${checked.size} / ${exercise.steps.length} etapas marcadas`}
            </button>
          </>
        ) : (
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold text-white">Quiz de validación</span>
          </div>
          {submitted && (
            <span
            ...
            >
              {isCorrect ? (
                <><svg width="9" height="9" viewBox="0 0 9 9" fill="none"><path d="M1.5 4.5l2.5 2.5 4-4" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" /></svg>Respuesta correcta</>
              ) : (
                <><svg width="9" height="9" viewBox="0 0 9 9" fill="none"><path d="M7 2L2 7M2 2l5 5" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" /></svg>Respuesta incorrecta</>
              )}
            </span>
          )}
        </div>
        ...
        {!submitted && (
          ...
          >
            {selected !== null ? "Validar mi respuesta" : "Elige una respuesta"}
          </button>
        )}
          ...
          >
            ...
            <div>
              <p className={`text-sm font-semibold mb-1 ${isCorrect ? "text-emerald-400" : "text-red-400"}`}>
                {isCorrect ? "¡Excelente respuesta!" : "No del todo."}
              </p>
...
function CompletionScreen() {
  ...
  return (
    ...
    >
      ...
      <p className="text-[11px] font-semibold tracking-widest uppercase text-emerald-500/70 mb-2">Lección terminada</p>
      ...
      <p className="text-zinc-400 text-sm leading-relaxed mb-6 max-w-xs mx-auto">
        Contenido leído · Puntos asimilados · Ejercicio completado · Quiz validado
      </p>
      ...
      <div className="flex items-center justify-center gap-6 mb-7">
        {[
          { label: "Lección", value: "1/9" },
          { label: "Nivel",   value: "Principiante" },
          { label: "Racha",   value: "1 día 🔥" },
        ].map((s) => (
          ...
          <div className="min-w-0">
            ...
            <FadeIn delay={0}>
              <div className="mb-8">
                <div className="flex items-center gap-2 text-[11px] text-zinc-600 mb-4">
                  ...
                  <span>/</span>
                  <Link href="/formations" className="hover:text-zinc-400 transition-colors">Principiante</Link>
                  <span>/</span>
            ...
            <div id="lesson-content">
              <FadeIn delay={60}>
                <div className="flex items-center gap-3 mb-5">
                  <p className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest">Lectura</p>
            ...
            <FadeIn delay={120 + sc * 90 + 60}>
              <div className="flex items-center gap-4 py-2 mb-6">
                ...
                <span className="text-[10px] font-semibold text-zinc-700 uppercase tracking-widest">Repaso</span>
                <div className="flex-1 h-px bg-zinc-800" />
              </div>
            </FadeIn>
            ...
            <div className="mt-10 pt-6 border-t border-zinc-800/60 flex items-center justify-between">
              <span className="text-xs text-zinc-700">Lección 1 de 9</span>
              <Link
                href="/formations/debutant/lecon2"
