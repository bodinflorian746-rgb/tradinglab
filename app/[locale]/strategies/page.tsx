...
const LEVEL_CLASSES: Record<StrategyLevel, {
  badgeClass:  string;
  dotClass:    string;
  labelClass:  string;
}> = {
  debutant:      { badgeClass: "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20", dotClass: "bg-emerald-500", labelClass: "text-emerald-400" },
  intermediaire: { badgeClass: "bg-blue-500/10 text-blue-400 border border-blue-500/20",          dotClass: "bg-blue-400",    labelClass: "text-blue-400"    },
  avance:        { badgeClass: "bg-amber-400/10 text-amber-400 border border-amber-400/20",       dotClass: "bg-amber-400",   labelClass: "text-amber-400"   },
};

// ─── Module card ───────────────────────────────────────────────────────────────

function ModuleCard({ module, locale, t }: { module: StrategyModule; locale: Locale; t: StratDict }) {
  const cfg = LEVEL_CLASSES[module.level];
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-zinc-500 leading-relaxed flex-1">{mod?.subtitle ?? module.subtitle}</p>
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="text-center mb-14">
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-5 leading-tight">
            ...
            <br />
            <span className="text-zinc-500">{t.page.hero.titleLine2}</span>
          </h1>
            ...
            return (
              <div key={level} className="flex items-center">
                <div className="flex flex-col items-center text-center px-2 sm:px-6 py-3 rounded-xl">
                  ...
                  <span className={`text-[10px] font-medium mt-0.5 ${cfg.labelClass}`}>{lvl.tagline}</span>
