...
export default function DashboardPage() {
  ...
  const trF = (id: string, fallback: string | undefined) => fdict[id]?.title ?? fallback ?? id;
  const trL = (fid: string, lid: string, fallback: string) =>
    fdict[fid]?.lessons?.[lid]?.title ?? fallback;
          ...
          <>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-zinc-500 mb-0.5">
                      {trF(active.formation.id, active.formation.title)}
                    </p>
                    <h2 className="text-lg font-bold mb-4 leading-snug">
                      {trL(active.formation.id, active.lesson.id, active.lesson.title).replace(/Leçon \d+ : /, "")}
                    </h2>
                    {(() => {
            ...
            {!active && !hasStarted && (
              <div className="border border-zinc-800 rounded-2xl px-6 py-10 text-center">
                ...
                <Link
                  href={h("/formations/debutant/lecon1")}
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5">
                      ...
                      {formation.lessons.map((lesson) => {
                        ...
                        const lessonRow = (
                          ...
                          >
                            ...
                            >
                              {trL(formation.id, lesson.id, lesson.title).replace(/Leçon \d+ : /, "")}
                            </p>
                        ...
                        return (
                          <Link key={lesson.id} href={h(lesson.href)}>
                            {lessonRow}
                          </Link>
                        );
                      })}
                    ...
                    {allDone ? (
                      <Link
                        href={h(formation.lessons[0].href)}
                      ...
                      <Link
                        href={h(nextLesson?.href ?? formation.lessons[0].href)}
