...
export default function BuildTheTradePage() {
  ...
  const chart: BuildTradeChart | null = useMemo(
    () => (current ? buildBuildTradeChart(current, current.seed, current.volatility) : null),
    [current],
  );
  ...
  if (!difficulty || !seed) {
    return <DifficultyPicker onPick={(d) => {
      ...
      setDifficulty(d);
      setSeed(s);
      setScenarios(generateBuildTradeScenarios(s, d));
    }} />;
  }
    ...
    return (
      <Summary
        ...
        onReplay={() => {
          const s = Math.floor(Math.random() * 1e9) >>> 0;
          setSeed(s);
          setScenarios(generateBuildTradeScenarios(s, difficulty));
          setIdx(0);
          setScore(0);
          setStreak(0);
          setMaxStreak(0);
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        {streak > 0 && (
          <div className="mb-4 flex items-center justify-center gap-1.5 text-[11px] font-semibold">
            ...
            <span className="text-zinc-500">setups parfaits consécutifs</span>
          </div>
        )}
        ...
        {isReveal && (
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl px-4 py-5 text-center">
            ...
            <p className="text-sm text-zinc-300">
              On regarde comment le marché se comporte avec ton plan…
            </p>
          </div>
        )}
        ...
        <div className="mt-6 pt-5 border-t border-zinc-800/60 grid grid-cols-3 gap-2.5">
          <StatTile label="Setups parfaits" value={`${stats.perfectSetups}`} />
          <StatTile label="TP atteints"     value={`${stats.tpHits}`} />
          <StatTile label="SL touchés"      value={`${stats.slHits}`} />
        </div>
      </div>
    </main>
  );
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white">
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Construis le setup</h1>
        <p className="text-zinc-400 text-sm leading-relaxed mb-8">
          {ROUNDS_PER_SESSION} scénarios. Pour chacun, tu choisis l'entrée, le stop loss et le take profit.
          Le marché révèle ensuite ce qui s'est passé — RR, drawdown, verdict.
        </p>
...
function BuildPanel({
  chart, entry, stop, tp, onEntry, onStop, onTp, onValidate, canValidate,
}: {
    ...
    <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl overflow-hidden">
      ...
      <div className="px-4 py-4 border-t border-zinc-800/60">
        {canValidate && entry && stop && tp && (
          <RrPreview chart={chart} entry={entry} stop={stop} tp={tp} />
        )}
        <button
          onClick={onValidate}
...
function RrPreview({ chart, entry, stop, tp }: { chart: BuildTradeChart; entry: EntryType; stop: StopType; tp: TpType }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2 mb-3 text-center">
      <RrTile label="Risque" value={risk.toFixed(2)} colorClass="text-zinc-300" />
      <RrTile label="Reward" value={reward.toFixed(2)} colorClass="text-zinc-300" />
      <RrTile label="R/R" value={rr > 0 ? `1:${rr.toFixed(1)}` : "—"} colorClass={rrColor} />
    </div>
  );
}
...
function Feedback({
  result, chart, picks, optimal, title, optimalExplain, lesson, difficulty, onNext, isLast,
}: {
  ...
  const verdict = setupVerdict(result);
  const border =
      ...
      <div className="px-4 py-4">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <OutcomeTile
            label="Outcome"
            value={
              result.outcome === "tp_hit"   ? "TP atteint ✓"
            : result.outcome === "sl_hit"   ? "SL touché ✗"
            : result.outcome === "no_fill"  ? "Pas d'entrée"
            :                                  "Trade ouvert"
            }
            colorClass={
        ...
        {result.entryFilled && (
          <div className="grid grid-cols-2 gap-2 mb-3">
            <OutcomeTile label="Drawdown" value={result.maxDrawdown.toFixed(2)} colorClass="text-zinc-300" />
            <OutcomeTile
              label="Plan optimal"
        ...
        <div className="space-y-1.5 mb-3">
          <ChoiceRow label="Entrée"    user={ENTRY_LABELS[picks.entry]}   best={ENTRY_LABELS[optimal.entry]}  match={picks.entry === optimal.entry} />
          <ChoiceRow label="Stop"      user={STOP_LABELS[picks.stop]}     best={STOP_LABELS[optimal.stop]}    match={picks.stop  === optimal.stop} />
          <ChoiceRow label="Take Prof" user={TP_LABELS[picks.tp]}         best={TP_LABELS[optimal.tp]}        match={picks.tp    === optimal.tp} />
        </div>

        <div className="bg-blue-500/8 border border-blue-500/25 rounded-lg px-3 py-2.5 mb-3">
          <p className="text-[10px] font-bold text-blue-400 uppercase tracking-wide mb-1">Setup optimal</p>
        ...
        <div className="bg-amber-500/8 border border-amber-500/25 rounded-lg px-3 py-2.5 mb-3">
          <p className="text-[10px] font-bold text-amber-400 uppercase tracking-wide mb-1">Leçon · {DIFFICULTY_META[difficulty].label}</p>
...
function Summary({
  ...
  const verdict = sessionVerdict(score, stats.perfectSetups, total);
  const verdictColor =
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">{ROUNDS_PER_SESSION} setups construits</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-8">
          ...
          <BigStat label="Setups parfaits"  value={`${stats.perfectSetups}/${total}`}     valueClass="text-emerald-400" />
          <BigStat label="TP atteints"      value={`${stats.tpHits}`}                     valueClass="text-emerald-400" />
          <BigStat label="SL touchés"       value={`${stats.slHits}`}                     valueClass={stats.slHits > 0 ? "text-red-400" : "text-zinc-300"} />
        </div>
